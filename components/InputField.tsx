...
const InputField = ({ label, labelStyle, secureTextEntry=false, containerStyle, icon, iconStyle, inputStyle, className, ...props }: InputFieldProps) => {
  return (
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View className="my-2 w-full">
                ...
                <View className={`flex flex-row justify-start items-center relative bg-neutral-100 rounded-full border border-neutral-100 focus:border-primary-500 ${containerStyle}`}>
                    {icon && (<Image source={icon} className={`h-6 w-6 ml-4 ${iconStyle}`} />)}
                    <TextInput 
